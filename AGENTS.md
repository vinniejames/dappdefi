...
The user (`vinniejames` / Vince DePalma) is co-founder of [Rocko](https://rocko.co) and runs this directory as a side project. Rocko, Kairos, and CoinFox are featured prominently; treat the user as a practitioner with deep DeFi expertise, not a beginner.
...
scripts/
  ...
  build-featured.ts       # rebuild featured.json (rarely needed)

public/
  logos/*                 # self-hosted logos (rocko.png, kairos.svg, coinfox.png, etc.)
  CNAME                   # dappdefi.com
  robots.txt
  llms.txt
  og.png + og.svg
...
The user has additional internal tooling and discovery workflows that aren't documented in this public file — see `~/.claude/projects/-Users-vince-Sites-dappdefi/memory/` for those notes.
...
Default for the seed protocols is `nofollow`. `link_type: follow` in frontmatter strips nofollow off the website / twitter / github links so the protocol gets editorial PageRank. Reserve for features and paid placements.
...
- **Description body**: 2–3 paragraphs. First paragraph is rendered as a lead (larger, bolder via `.protocol-body > p:first-child`). Always write in your own words — never paste marketing copy verbatim. Mention close peers when it helps frame the protocol's position.
...
- **`other` category**: last resort. Almost everything fits one of the 14 named categories.
...
- **`listed_at: 2026-04-29`** is the date the entry was added (renamed in UI to "Last Updated"). Don't change it on edits — historical record. Higher resolution timestamps (`2026-04-29T18:00:00Z`) sort entries within the same day for the homepage's Recently Added strip.
...
- Repo is at `vinniejames/dappdefi`. Site is at `https://dappdefi.com` (DNS at Namecheap, GitHub Pages with Let's Encrypt cert, https_enforced=true).
...
- Don't run risky or visible-state actions (visibility flips, force pushes, public publishing) without explicit permission. The user controls the moment of public release.
...
- Per-batch commits during long content runs (recoverable from `git log`).
